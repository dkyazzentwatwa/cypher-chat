...
void TerminalManager::cmdSend(const char* args) {
  ...
  if (buttonNum < 1 || buttonNum > 4) {  // Fixed: use 4 instead of NUM_BUTTONS for S3 profile version
...
void TerminalManager::cmdGPS() {
  ...
  output.println("GPS is enabled but no GPS manager in S3 profile version");
#else
  output.println("GPS is disabled (GPS_ENABLED=false in Config_S3.h).");
  output.println("S3 profiles use board-specific display/input adapters.");
#endif
}
