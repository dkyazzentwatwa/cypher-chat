...
bool DisplayPort::begin() {
  ...
  return true;
#elif BOARD_HAS_DISPLAY && BOARD_DISPLAY_IS_SSD1306
  constexpr uint8_t candidateAddresses[] = {SSD1306_I2C_ADDR, 0x3D};
#if BOARD_PROFILE == BOARD_PROFILE_XIAO_ESP32C3_OLED_GPS_3BTN
  constexpr int candidatePins[][2] = {
    {PIN_SSD1306_SDA, PIN_SSD1306_SCL},
    {SDA, SCL},
  };
#else
  constexpr int candidatePins[][2] = {
  ...
  for (size_t pinIndex = 0; pinIndex < sizeof(candidatePins) / sizeof(candidatePins[0]) && !_ready; pinIndex++) {
    const int sda = candidatePins[pinIndex][0];
    const int scl = candidatePins[pinIndex][1];
    Serial.printf("SSD1306 I2C try SDA=%d SCL=%d\n", sda, scl);
    Wire.end();
    Wire.begin(sda, scl);
    ...
    for (uint8_t address : candidateAddresses) {
      ...
      Serial.printf("SSD1306 probe 0x%02X -> %u\n", address, error);
      if (error == 0) {
        _ready = _oled.begin(SSD1306_SWITCHCAPVCC, address);
        Serial.printf("SSD1306 begin 0x%02X -> %d\n", address, _ready ? 1 : 0);
        if (_ready) {
          break;
        }
      }
