...
void OutputManager::begin() {
  // USB is initialized by UsbConsole; BLE starts later and is checked per write.
  _usbEnabled = true;
  _btEnabled = true;
}
...
size_t OutputManager::print(const char* str) {
  ...
  if (_usbEnabled) {
    bytesWritten = usbConsole.print(str);
  }
...
size_t OutputManager::print(char c) {
  if (_usbEnabled) usbConsole.print(c);
#if BLE_UART_ENABLED
...
size_t OutputManager::print(int n) {
  if (_usbEnabled) usbConsole.print(n);
#if BLE_UART_ENABLED
  if (_btEnabled && bleUARTMgr.isConnected()) {
    char buf[16];
...
size_t OutputManager::print(const __FlashStringHelper* f) {
  if (_usbEnabled) usbConsole.print(f);
#if BLE_UART_ENABLED
  if (_btEnabled && bleUARTMgr.isConnected()) {
...
size_t OutputManager::println(const char* str) {
  ...
  if (_usbEnabled) {
    bytesWritten = usbConsole.println();
  }
...
size_t OutputManager::write(uint8_t c) {
  ...
  if (_usbEnabled) {
    bytesWritten = usbConsole.write(c);
  }
...
size_t OutputManager::write(const uint8_t* buffer, size_t size) {
  ...
  if (_usbEnabled) {
    bytesWritten = usbConsole.write(buffer, size);
  }
