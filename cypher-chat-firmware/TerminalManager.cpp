...
void TerminalManager::begin() {
  ...
  ansiEnabled = true;
  lastUpdate = 0;
  usbConnected = usbConsole.connected();
  lastUsbPromptMs = millis();
  ...
  output.println();
  output.println("Command mode ready. Type 'help' for commands or 'menu' for menu mode.");
...
void TerminalManager::poll() {
  ...
  refreshUsbAttachPrompt();

  // Priority: check USB console first
  while (usbConsole.available()) {
    int c = usbConsole.read();
    if (c < 0) {
      break;
    }
    processInput(static_cast<char>(c), INPUT_SOURCE_USB);
  }
...
void TerminalManager::refreshUsbAttachPrompt() {
  const bool connected = usbConsole.connected();
  const unsigned long now = millis();

  if (connected && !usbConnected && now - lastUsbPromptMs > 750) {
    usbConsole.println();
    usbConsole.println("Cypher Chat USB terminal connected.");
    if (menuState == MENU_NONE) {
      printPrompt();
    } else {
      usbConsole.println("Menu mode active. Press 0 to return.");
    }
    lastUsbPromptMs = now;
  }
  ...
  if (menuState != MENU_NONE) {
    ...
    if (c == '\n' || c == '\r') {
      if (inputPos > 0) {
        ...
        if (echoToUSB) usbConsole.println();
#if BLE_UART_ENABLED
        if (echoToBT) {
          bleUARTMgr.write('\r');
          bleUARTMgr.write('\n');
      ...
      if (inputPos > 0) {
        ...
        if (echoToUSB) usbConsole.print("\b \b");
#if BLE_UART_ENABLED
        if (echoToBT) {
          bleUARTMgr.write('\b');
          bleUARTMgr.write(' ');
      ...
      if (echoToUSB) usbConsole.println("^C");
#if BLE_UART_ENABLED
      if (echoToBT) {
        bleUARTMgr.write('^');
        bleUARTMgr.write('C');
      ...
      if (echoToUSB) usbConsole.print(inputBuffer);
#if BLE_UART_ENABLED
      ...
      if (echoToUSB) usbConsole.write(static_cast<uint8_t>(c));
#if BLE_UART_ENABLED
...
void TerminalManager::handleTabCompletion() {
  ...
  if (matchCount == 0) {
    ...
    for (int i = 0; i < matchCount; i++) {
      ...
      if (matches[i]->alias) {
        usbConsole.print(" (");
        usbConsole.print(matches[i]->alias);
        usbConsole.print(")");
      }
      usbConsole.println();
    }
    printPrompt();
    usbConsole.print(inputBuffer);  // Restore input
  }
}
...
void TerminalManager::cmdSend(const char* args) {
  ...
  if (buttonNum < 1 || buttonNum > 4) {  // Fixed: use 4 instead of NUM_BUTTONS for unified version
...
void TerminalManager::cmdHelp(const char* args) {
  ...
  output.println("\n================================================");
  output.println("  CYPHER-CHAT S3 TERMINAL COMMANDS");
  output.println("================================================\n");
...
void TerminalManager::cmdVersion() {
  output.println();
  printColored("================================================\n", COLOR_CYAN);
  printColored("     CYPHER-CHAT S3 Firmware                 \n", COLOR_CYAN);
  printColored("================================================\n", COLOR_CYAN);
  ...
  output.println(BOARD_PROFILE_NAME);
...
void TerminalManager::cmdGPS() {
  ...
  output.println("Unified profiles use board-specific display/input/GPS adapters.");
#endif
}
...
void TerminalManager::displayMainMenu() {
  ...
  output.println("================================================");
  output.println("        CYPHER-CHAT S3 Control Menu          ");
  output.println("================================================");
...
void TerminalManager::displayMonitorDashboard() {
  ...
  output.println("================================================");
  output.println(" CYPHER CHAT S3 - LIVE MONITOR               ");
  output.println("================================================");
...
void TerminalManager::printBanner() {
  ...
  output.println();
  output.println("================================================");
  output.println("    CYPHER CHAT S3 - Emergency Communication  ");
...
void TerminalManager::printStatus() {
  output.println("\n================================================");
  output.println("  CYPHER CHAT S3 - Status");
  output.println("================================================");
...
void TerminalManager::printMessageHistory() {
  ...
  for (int i = 0; i < count; i++) {
    ...
    output.print(i + 1);
    output.print(". ");
    output.println(messageHistory[idx]);
  }
  output.println();
}
