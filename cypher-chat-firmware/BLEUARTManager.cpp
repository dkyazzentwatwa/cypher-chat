...
size_t BLEUARTManager::write(const uint8_t* buffer, size_t size) {
  ...
  // Keep notifications conservative. Many phone BLE terminal apps still behave
  // best with 20-byte UART-style packets, even when larger MTUs are available.
  ...
  while (sent < size) {
    ...
    // Pace every notification so multi-line command output is not dropped.
    delay(12);
  }
