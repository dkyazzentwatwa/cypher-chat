...
extern uint8_t savedMeshChannel;
extern const char* fieldModeLabel();
extern void fieldConsoleDump();
extern void fieldConsoleLogs();
extern void fieldConsoleResetSettings();
extern void fieldConsoleSetBrightness(const char* args);
extern void fieldConsoleSetSpeaker(const char* args);
extern void fieldConsoleSetRelayMode(const char* args);
extern void fieldConsoleSetPeerName(const char* args);
extern void fieldConsoleSetPeerTrust(const char* args);
...
const CommandDesc TerminalManager::commands[] = {
  ...
  {"brightness", "br",   "<40-255>", "Set Cardputer display brightness",                    CAT_CONFIG},
  {"speaker",   "spk",   "<on/off>", "Enable or mute Cardputer speaker",                    CAT_CONFIG},
  {"relay",     nullptr, "<mode>",   "Set field mode: normal/quiet/monitor/base",           CAT_CONFIG},
  {"peername",  nullptr, "<suffix> <name>", "Save local peer nickname",                     CAT_CONFIG},
  {"trust",     nullptr, "<suffix> <level>", "Set peer trust label",                        CAT_CONFIG},
  ...
  {"dump",      nullptr, nullptr,   "Export settings, peers, stats, and message counts",     CAT_SYSTEM},
  {"logs",      nullptr, nullptr,   "Print recent local message log",                        CAT_SYSTEM},
  {"resetsettings", nullptr, nullptr, "Clear saved Cardputer settings and restart",           CAT_SYSTEM},
};
...
TerminalMode TerminalManager::getMode() const {
  return mode;
}

bool TerminalManager::isAnsiEnabled() const {
  return ansiEnabled;
}

void TerminalManager::setAnsiEnabled(bool enabled) {
  ansiEnabled = enabled;
  saveConfig();
}
...
void TerminalManager::executeCommand(const char* verb, const char* args) {
  ...
  // System commands
    ...
    cmdLogs();
  } else if (strcmp(cmd, "resetsettings") == 0) {
    cmdResetSettings();
  }
  // Unknown command
  else {
...
void TerminalManager::cmdSend(const char* args) {
  ...
  if (buttonNum < 1 || buttonNum > 4) {  // Fixed: use 4 instead of NUM_BUTTONS for Cardputer version
...
void TerminalManager::cmdHelp(const char* args) {
  ...
  output.println("\n================================================");
  output.println("  CYPHER-CHAT CARDPUTER TERMINAL COMMANDS");
  output.println("================================================\n");
...
void TerminalManager::cmdVersion() {
  output.println();
  printColored("================================================\n", COLOR_CYAN);
  printColored("     CYPHER-CHAT CARDPUTER Firmware                 \n", COLOR_CYAN);
  printColored("================================================\n", COLOR_CYAN);

  output.print("Version:    ");
  printColored(FIRMWARE_VERSION "\n", COLOR_GREEN);

  output.print("Build:      ");
  output.println(__DATE__ " " __TIME__);

  output.print("Platform:   ");
  output.println("M5Stack Cardputer-Adv");
...
void TerminalManager::cmdBrightness(const char* args) {
  fieldConsoleSetBrightness(args);
}

void TerminalManager::cmdSpeaker(const char* args) {
  fieldConsoleSetSpeaker(args);
}

void TerminalManager::cmdRelay(const char* args) {
  fieldConsoleSetRelayMode(args);
}

void TerminalManager::cmdPeerName(const char* args) {
  fieldConsoleSetPeerName(args);
}

void TerminalManager::cmdTrust(const char* args) {
  fieldConsoleSetPeerTrust(args);
}

void TerminalManager::cmdDump() {
  fieldConsoleDump();
}
...
void TerminalManager::cmdResetSettings() {
...
void TerminalManager::cmdGPS() {
  ...
  output.println("GPS is enabled but no GPS manager in Cardputer version");
#else
  output.println("GPS is disabled (GPS_ENABLED=false in Config_Cardputer.h).");
  output.println("Cardputer-Adv v1 uses onboard keyboard/display only.");
#endif
}
...
void TerminalManager::displayMainMenu() {
  ...
  output.println("================================================");
  output.println("        CYPHER-CHAT CARDPUTER Control Menu          ");
  output.println("================================================");
...
void TerminalManager::displayMonitorDashboard() {
  ...
  output.println("================================================");
  output.println(" CYPHER CARDPUTER - LIVE MONITOR               ");
  output.println("================================================");
...
void TerminalManager::printBanner() {
  ...
  output.println();
  output.println("================================================");
  output.println("    CYPHER CARDPUTER - Emergency Communication  ");
  output.println("    Keyboard, display, USB, BLE, and ESP-NOW    ");
  output.println("================================================");
}
...
void TerminalManager::printStatus() {
  output.println("\n================================================");
  output.println("  CYPHER CARDPUTER - Status");
  output.println("================================================");
...
void TerminalManager::printMessageHistory() {
  output.println("\nMessage History:");
  output.println("================");
  int count = min(messageStore.count(MSG_FILTER_ALL), 16);
  if (count == 0) {
    output.println("(empty)");
  }
  for (int i = 0; i < count; i++) {
    char line[140];
    if (messageStore.formatNewest(i, MSG_FILTER_ALL, line, sizeof(line))) {
      output.print(i + 1);
      output.print(". ");
      output.println(line);
    }
...
void TerminalManager::printConfiguration() {
  ...
  output.print("Field Mode: ");
  output.println(fieldModeLabel());
  output.print("Brightness: ");
  output.println(fieldBrightness);
  output.print("Speaker: ");
  output.println(speakerMuted ? "muted" : "on");
  output.print("Saved Mesh Channel: ");
  output.print(savedMeshChannel);
  output.print(" (active ");
