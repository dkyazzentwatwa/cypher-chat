...
**Note:** This project has compilation issues due to missing Server.cpp/Client.cpp files referenced in cypher-chat.ino. The old BLE server/client architecture has been removed but references remain.
...
**cypher-chat** is an ESP32-based mesh networking system for emergency communications with wireless terminal access. It uses **ESP-NOW** for multi-hop message relay (250m per hop, up to 5 hops) and provides terminal access via USB Serial or Bluetooth Serial from mobile apps.
...
**cypher-chat.ino** (Main entry point)
...
**Config.h** (Hardware and feature flags)
...
4. Invoke `onMeshMessage()` callback in cypher-chat.ino
...
1. **Compilation broken:** `cypher-chat.ino` includes `Server.h` and `Client.h` (lines 12-13) which don't exist. Remove these includes and the calls to `setupServer()`, `setupClient()`, `loopServer()`, `clientLoop()` (lines 470-474, 517-519).
...
3. **Missing pin definitions:** `BUZZER_PIN`, `LED_PIN_R/G/B` not defined in Config.h but referenced in code. Currently handled with `#if BUZZER_PIN != -1` guards.
